...
def format_example(example, columns, format_dict):
    return urllib.parse.unquote(
        "\n".join([f"{format_dict[col](example[col])}" for col in columns])
    )


def tokenize_dataset(split_dataset, config, num_proc=1, debug_print=False):
    """Tokenize the dataset."""
    columns = config["format"].keys()
    format_dict = config["format"]
    hf_token = os.getenv("HF_TOKEN", None)
    enc = LlamaTokenizer.from_pretrained(
        "meta-llama/Llama-2-7b", token=hf_token, add_eos_token=True, use_fast=True
    )

    def process(example):
        text = format_example(example, columns, format_dict)
        ids = enc.encode(text)
    ...
    if debug_print:
        for i in range(1):
            x = format_example(split_dataset["train"][i], columns, format_dict)
            print("-" * 100)
            print(x)
            print("-" * 100)

    tokenized = split_dataset.map(
        process, remove_columns=columns, desc="tokenizing the splits", num_proc=num_proc
    )

    return tokenized


def save_to_binary(tokenized_dataset, output_file, total_batches=1024):
    """Save the dataset in binary format using memory-mapped files."""

    arr_len = np.sum(tokenized_dataset["len"], dtype=np.uint64)
    dtype = np.uint16  # Assuming that the maximum token value is < 2**16

    arr = np.memmap(output_file, dtype=dtype, mode="w+", shape=(arr_len,))

    # Ensure the number of batches does not exceed the number of samples in the dataset
    total_batches = min(total_batches, len(tokenized_dataset))
...
if __name__ == "__main__":
    ...
    for key in config.keys():
        ...
        tokenized_data = tokenize_dataset(split_data, selected_config, num_proc, True)
        ...
        # Save using the binary approach
        if not os.path.exists(key):
            os.mkdir(key)
        print("Saving results...")

        save_to_binary(results_train, f"{key}/{key}_train.bin")
        save_to_binary(results_val, f"{key}/{key}_val.bin")
